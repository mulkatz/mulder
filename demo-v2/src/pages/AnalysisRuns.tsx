...
const runColumns: DataColumn<AnalysisRun>[] = [
  ...
  { key: 'credits', header: 'Credits', className: 'w-24', render: (run) => <span className="font-mono">{run.credits}</span> },
  { key: 'started', header: 'Started', className: 'w-32', render: (run) => <span className="font-mono text-xs">{run.startedAt}</span> },
];
  ...
  return (
    <>
      <PageHeader
        actions={
          <>
            <button className="inline-flex h-9 items-center gap-2 rounded-md border border-border bg-panel px-3 text-sm text-text transition-colors hover:bg-field" type="button">
      ...
      <div className="grid gap-4 p-4 sm:p-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <section className="panel min-w-0 overflow-hidden">
          <Toolbar className="gap-3">
            <SearchInput className="w-full sm:max-w-sm" onChange={(event) => setQuery(event.target.value)} placeholder="Filter runs..." value={query} />
        ...
        <InspectorPanel subtitle={selectedRun.id} title={selectedRun.title}>
          ...
          <InspectorSection title="Artifacts">
            <div className="space-y-2">
              {selectedRun.artifacts.map((artifact) => (
                <div className="flex items-center justify-between gap-3 rounded-md border border-border bg-panel-raised px-3 py-2" key={artifact.name}>
                  <div className="min-w-0">
          ...
          <InspectorSection title="Query">
            <p className="rounded-md border border-border bg-panel-raised p-3 text-sm text-text-muted">{selectedRun.query}</p>
          </InspectorSection>
