...
describe('Spec 01: Demo App — Evidence Analysis UI', () => {
  ...
  it('QA-05: Mock data includes spatio-temporal events with coordinates and timestamps', () => {
    ...
    expect(
      content.match(/lat\b/),
      'SpatioTemporalEvent must include latitude field'
    ).not.toBeNull();

    expect(
      content.match(/lng\b/),
      'SpatioTemporalEvent must include longitude field'
    ).not.toBeNull();

    // Must include timestamp field
    expect(
      content.match(/timestamp|date/i),
      'SpatioTemporalEvent must include timestamp or date field'
    ).not.toBeNull();
  ...
  it('QA-06: Layout includes Evidence navigation item with correct route', () => {
    ...
    expect(
      content.match(/Shield/),
      'Layout must reference Shield icon for Evidence nav item'
    ).not.toBeNull();
  });
  ...
  it('QA-06b: App.tsx registers /evidence route pointing to Evidence component', () => {
    ...
    expect(
      content.match(/import.*Evidence.*from/),
      'App.tsx must import the Evidence page component'
    ).not.toBeNull();

    // Must register /evidence route
    expect(
      content.match(/['"\/]evidence['"]/),
      'App.tsx must register the /evidence route'
    ).not.toBeNull();
  });
  ...
  it('QA-08: Existing pages still exist and build succeeds (no regressions)', () => {
    ...
    for (const route of existingRoutes) {
      ...
      expect(
        appContent.match(routePattern),
        `App.tsx must still contain route for ${route}`
      ).not.toBeNull();
    }
  });
